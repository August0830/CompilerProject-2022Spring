...
StructHead StructSpecifier(Node *node)
{
    ...
    if (!strcmp(child->next->name, "OptTag"))
    {
        ...
        isInStruct = false; //use serialized property
...
SymbolItem Dec(Node *node, Type type)
{
    ...
    if (!strcmp(child->name, "VarDec"))
    {
        ...
        if (child->next != NULL && !strcmp(child->next->name, "ASSIGNOP") && child->next->next != NULL && !strcmp(child->next->next->name, "Exp"))
        {
            ...
            if (val == NULL)
                return NULL;
            if (isSameType(var->type, val->type))
            {
                var->val = val->val;
            }
            else
...
SymbolItem VarDec(Node *node, Type type)
{
    ...
    {
        isInArrayDef = true;
        SymbolItem idItem = VarDec(child, type);
        ...
        {
            arrayType->info.arrayInfo.size = sizeNode->val.iVal;
        }
        else
            arrayType->info.arrayInfo.size = 0;
        var->type = arrayType;
        var->name = newString(idItem->name);
        isInArrayDef = false;
    }
    else if (!strcmp(child->name, "ID")) //VarDec : ID
    {
        var->name = newString(child->val.val);
        if (var->type->kind != T_STRUCT)
            var->type->kind = T_ID;
    }
...
SymbolItem Exp(Node *node)
{
        ...
        else
        {
            if (!strcmp(child->next->name, "LP")) //function
            {
                ...
                if (funcInfo == NULL)
                {
                    ...
                    {
                        char msg[100] = {0};
                        sprintf(msg, "Undefined function \"%s\".", child->name);
    ...
    else if (!strcmp(child->name, "Exp"))
    {
        ...
        if (!strcmp(child->next->name, "ASSIGNOP"))
        {
            ...
            else if (!isSameType(e1->type, e2->type))
            {
                char msg[100] = {0};
                sprintf(msg, "Type mismatched for assignment.");
                pError(TYPE_MISMATCHED_ASSIGN, node->lineNum, msg);
        ...
        else if (!strcmp(child->next->name, "AND") || !strcmp(child->next->name, "OR") || !strcmp(child->next->name, "RELOP") || !strcmp(child->next->name, "PLUS") || !strcmp(child->next->name, "SUB") || !strcmp(child->next->name, "STAR") || !strcmp(child->next->name, "DIV"))
        {
            ...
            if (e2 == NULL)
                return NULL;
            if (e1->type->kind != BASIC || e2->type->kind != BASIC || !isSameType(e1->type, e2->type))
            {
                char msg[100] = {0};
            ...
            SymbolItem item = (SymbolItem)malloc(sizeof(SymbolItem_));
            char name[20] = {0};
            sprintf("\"%s\" at %d", child->next->val.val, node->lineNum);
            item->name = newString(name);
            item->type = e1->type;
            item->val = e1->val;
            item->next = NULL;
        ...
        else if (!strcmp(child->next->name, "LB"))
        {
            ...
            if (index->type->kind != BASIC || index->type->info.basicType != B_INT)
            {
                char msg[100] = {0};
            ...
            SymbolItem item = (SymbolItem)malloc(sizeof(SymbolItem_));
            sprintf(item->name, "\"%s\" elem", e1->name);
            item->next = NULL;
        ...
        else if (!strcmp(child->next->name, "DOT"))
        {
            ...
            {
                if (!strcmp(fieldInfo->fieldVar->name, id))
                {
                    SymbolItem item = (SymbolItem)malloc(sizeof(SymbolItem_));
                    sprintf(item->name, "\"%s\".\"%s", e1->name, id);
                    item->next = NULL;
...
bool isMatchedFieldList(FieldList f1, FieldList f2)
{
    ...
    {
        if (!isSameType(f1->fieldVar->type, f2->fieldVar->type))
            return false;
        f1 = f1->nextField;
        f2 = f2->nextField;
    }
...
bool isSameType(Type t1, Type t2)
{
    ...
    {
        return t1->info.basicType == t2->info.basicType;
    }
    else if (t1->kind == ARRAY && t2->kind == ARRAY)
    {
        return isSameType(t1->info.arrayInfo.elemType, t2->info.arrayInfo.elemType);
    }
    else
        return false;
}
...
void Stmt(Node *node, Type returnType)
{
    ...
    else if (!strcmp(ch->name, "RETURN"))
    {
        ...
        if (returnVal == NULL)
            return;
        if (!isSameType(returnVal->type, returnType))
        {
            char msg[100] = {0};
